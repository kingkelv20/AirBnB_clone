...
class TestUser(unittest.TestCase):
    """ a test for the User calss in side user module """
    def test_attributes(self):
        """ tests the attributes of the class """
        u1 = User()
        dc = {"id": u1.id, "created_at": u1.created_at.isoformat(),
              "updated_at": u1.updated_at.isoformat(), "__class__": "User"}
    ...
    def test_str(self):
        ...
        b1 = User()
        self.assertEqual(str(b1), f"[User] ({b1.id}) {b1.__dict__}")
    ...
    def test_to_dict(self):
        ...
        dc = {'id': b1.id, 'created_at': b1.created_at.isoformat(),
              'updated_at': b1.updated_at.isoformat(),
              '__class__': 'User', 'name': 'my_name', 'my_number': 89}
        self.assertEqual(b1.to_dict(), dc)
        a = b1.to_dict()
        self.assertEqual(a['__class__'], "User")
    ...
    def test_user_attributes(self):
        ...
        u1.email = "someone@somewhere.com"
        dc = {'id': u1.id, 'created_at': u1.created_at.isoformat(),
              'updated_at': u1.updated_at.isoformat(),
              '__class__': 'User', "email": "someone@somewhere.com"}
        self.assertEqual(u1.to_dict(), dc)
        u1.password = "my_password"
        u1.first_name = "my_1st_name"
        u1.last_name = "my_last_name"
        dc = {'id': u1.id, 'created_at': u1.created_at.isoformat(),
              'updated_at': u1.updated_at.isoformat(),
              '__class__': 'User', "email": "someone@somewhere.com",
              "password": "my_password", "first_name": "my_1st_name",
